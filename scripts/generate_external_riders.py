...
def generate_html_table(df_licences):
    ...
    html_table = (
        ...
        "<th rowspan='2' class='align-middle'>Prénom</th>"
        "<th rowspan='2' class='align-middle'>Club</th>"
        "<th rowspan='2' class='align-middle'>Département</th>"
        "<th rowspan='2' class='align-middle'>Licence</th>"
        "<th colspan='4' class='text-center'>Catégories</th>"
        "<th rowspan='2' class='align-middle'>Date d'inscription</th>"
        "</tr>"
        "<tr>"
        "<th>FFC</th>"
        "<th>UFOLEP</th>"
        "<th>FSGT (extérieur)</th>"
        "<th>FSGT 71</th>"
        "</tr></thead>"
        "<tbody>"
    )
    for _, row in df_licences.iterrows():
        ...
        html_table += f"<td>{row['Département']}</td>"
        html_table += f"<td>{row['Numéro de licence FSGT']}</td>"
        if pd.notna(row["Catégorie FFC"]):
            html_table += f"<td>{row['Catégorie FFC']}</td>"
        else:
            html_table += f"<td></td>"
        if pd.notna(row["Catégorie UFOLEP"]):
            html_table += f"<td>{row['Catégorie UFOLEP']}</td>"
        else:
            html_table += f"<td></td>"
        html_table += f"<td>{row["Catégorie FSGT département d'origine"]}</td>"
        html_table += f"<td>{row['Catégorie autorisée FSGT 71']}</td>"
        html_table += f"<td>{row['Date d\'inscription'].strftime('%d/%m/%Y')}</td>"
        html_table += "</tr>"
